...
for repo = 1:c3
    ...
    stor_fill(repo, 1) = mean(S1(:, repo));
    if (isnan(mean(S1(:, repo))))
        stor_fill(repo, 1) = 0;
    end
    stor_fill(repo, 2) = mean(S2(:, repo));
    if (isnan(mean(S2(:, repo))))
        stor_fill(repo, 2) = 0;
    end
    stor_fill(repo, 3) = mean(S3(:, repo));
    if (isnan(mean(S3(:, repo))))
        stor_fill(repo, 3) = 0;
    end
    stor_fill(repo, 4) = mean(S4(:, repo));
    if (isnan(mean(S4(:, repo))))
        stor_fill(repo, 4) = 0;
    end
    stor_fill(repo, 5) = mean(S5(:, repo));
    if (isnan(mean(S5(:, repo))))
        stor_fill(repo, 5) = 0;
    end
    stor_fill(repo, 6) = mean(S6(:, repo));
    if (isnan(mean(S6(:, repo))))
        stor_fill(repo, 6) = 0;
    end
end
...
% lgd.FontSize = 9;
% xlabel('Repository number','fontsize',12) 
% ylabel('Bandwidth used (B/s)','fontsize',12)figure
...
% subplot(2,1,1);
% yyaxis left
bar_handle = bar([fresh_perc1, fresh_perc2, fresh_perc4, fresh_perc5, fresh_perc6]);
% title('Processing threads','fontsize',16)
xlabel('Repository number','fontsize',12) 
...
% subplot(2,1,1);
% yyaxis left
bar_handle = plot(1:80, U1, 1:80, U2, 1:80, U3, 1:80, U4, 1:80,  U5, 1:80,  U6);
% title('Processing threads','fontsize',16)
xlabel('Repository number','fontsize',12)
ylabel('No. of Unprocessed Messages/repo','fontsize',12)
lgd1 =legend('4:1', '3:1', '5:2', '4:2', '2:2', '2:1');
title(lgd1, 'Non-Proc:Proc');
lgd1.FontSize = 9;
lgd1.NumColumns = 5;
...
% subplot(2,1,1);
% yyaxis left
bar_handle = bar([U1', U2', U3', U4',  U5', U6']);
% title('Processing threads','fontsize',16)
xlabel('Repository number','fontsize',12)
ylabel('No. of Unprocessed Messages/repo','fontsize',12)
lgd1 =legend('4:1', '3:1', '5:2', '4:2', '2:2', '2:1');
...
bar([mean(sat_perc1), mean(sat_perc2), mean(sat_perc3), mean(sat_perc4), mean(sat_perc5), mean(sat_perc6)]);
xlabel('Repository number','fontsize',12) 
...
bar_handle = bar([stor_fill(:,1), stor_fill(:,2), stor_fill(:,3), stor_fill(:,4), stor_fill(:,5), stor_fill(:,6)]);
xlabel('Repository number','fontsize',12) 
ylabel('Percentage (*100%) of messages stored longer than shelf life','fontsize',12)
% xlim([17 48]);
set(bar_handle(1),'FaceColor',[0,0.5,1])
...
bar_handle = bar([mean(nonzeros(stor_fill(:,1))), mean(nonzeros(stor_fill(:,2))), mean(nonzeros(stor_fill(:,3))), mean(nonzeros(stor_fill(:,4))), mean(nonzeros(stor_fill(:,5))), mean(nonzeros(stor_fill(:,6)))]);
