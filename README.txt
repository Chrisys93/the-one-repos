...
large-scale proof-of-concept tool, for data and performance analysis and optimisation. The main subject of this proof-of-concept is the management and manipulation
...
simulations with different routing protocols, interfaces, storage, processing and 
delivery options, and visualizing both simulations interactively in real-time 
and results after their completion.


The folders in this repository include:

- file_support_v1.0 - repository structure developed based on this structure;

- one_1.4.1 - the main ONE-repos folder, with the compilable and executable 
code, variables and results;

- the-one-simple - the original ONE sumulator, with added changes to reports, 
for more convenient, starting results;
...
script, in either one_1.4.1 or the-one-simple. That should work both in 
Windows and Unix/Linux environment with Java 8 JDK or later.
...
- select from menus: New -> Java Project -> Navigate to either of the one_1.4.1 
or the-one-simple folders and load the folder as a project. Click Finish;
- Right-click on new project -> Build Path -> Add External Archives... -> Add the 
files from the associated project folder's lib directory;
- All set up.
...
then key name. Key and value are separated by an equals-sign. Namespaces
...
suffixed by two percent (%) signs. These placeholders are replaced by the
...
seed 2, etc. 
Note that you have to run it using batch mode (-b option) if
...
--
New, processing- and storage-specific group settings:
--

storageCapability
Whether the nodes in the group have storage capability. This is a boolean
value of true or false and if it is not set, the default value is false.

processingCapability
Whether the nodes in the group have processing capability. This is a boolean
value of true or false and if it is not set, the default value is false.

storageSize
Total size of storage capacity of each node in the group (bytes). Storage is 
comprised of two dynamically allocated storages: processing and non-processing (static). 
When the storage is full, the node sends new non-processing messages to be stored in the 
cloud, or new processing messages are processed ASAP, and if the processed messages cache
is also full, the node offloads the newest processing messages directly to the cloud.

compressionRate
The factor by which input data is divided, to be compressed. This factor also determines
the size of the processed cache size.

Two new applications have also been developed for the generation and management of messages,
according to the template provided by the original pingApp, and their configuration parameters 
are:

- For ProcApplication:
	procApp.depletionRate
The maximum rate at which the processing application is configured to deplete the storage of
a repository.
	procApp.passive
A boolean key-value pair, which determines whether the node should (or not) only store messages.
	procApp.coreNo
The number of processing cores the repository node should have.

- For ProcGenApplication:
	procGenApp.interval
The interval between generating two consecutive messages.
	procGenApp.delay
The processing delay expected for the processing messages generated by this application.
	procGenApp.destinationName
The destination group name intended for the messages generated by this application.
	procGenApp.procSize
The size of messages generated by this application.
	procGenApp.passiveRate
The ratio between the types of messages generated by this application (in the 
non-processing msg. no,processing msg. no ratio format). Default is 1:1.
	procGenApp.passive
Determines whether to generate only non-processing messages or both processing and non-processing.
Default is false.
