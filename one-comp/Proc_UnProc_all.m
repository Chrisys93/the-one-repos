...
for repo = 1:c3
    ...
    hproc_up(repo, 5) = mean(HPB5(:, repo))/(mean(HUB5(:, repo))+mean(HPB5(:, repo)));
    if (isnan(mean(HPB5(:, repo))/(mean(HUB5(:, repo))+mean(HPB5(:, repo)))))
        hproc_up(repo, 5) = 0;
    end
    ...
    huproc_up(repo, 1) = mean(HUB1(:, repo))/(mean(HUB1(:, repo))+mean(HPB1(:, repo)));
    if (isnan(mean(HUB1(:, repo))/(mean(HUB1(:, repo))+mean(HPB1(:, repo)))))
        huproc_up(repo, 1) = 0;
    end
    huproc_up(repo, 2) = mean(HUB2(:, repo))/(mean(HUB2(:, repo))+mean(HPB2(:, repo)));
    if (isnan(mean(HUB2(:, repo))/(mean(HUB2(:, repo))+mean(HPB2(:, repo)))))
        huproc_up(repo, 2) = 0;
    end
    huproc_up(repo, 3) = mean(HUB3(:, repo))/(mean(HUB3(:, repo))+mean(HPB3(:, repo)));
    if (isnan(mean(HUB3(:, repo))/(mean(HUB3(:, repo))+mean(HPB3(:, repo)))))
        huproc_up(repo, 3) = 0;
    end
    huproc_up(repo, 4) = mean(HUB4(:, repo))/(mean(HUB4(:, repo))+mean(HPB4(:, repo)));
    if (isnan(mean(HUB4(:, repo))/(mean(HUB4(:, repo))+mean(HPB4(:, repo)))))
        huproc_up(repo, 4) = 0;
    end
    huproc_up(repo, 5) = mean(HUB5(:, repo))/(mean(HUB5(:, repo))+mean(HPB5(:, repo)));
    if (isnan(mean(HUB5(:, repo))/(mean(HUB5(:, repo))+mean(HPB5(:, repo)))))
        huproc_up(repo, 5) = 0;
    end
    huproc_up(repo, 6) = mean(HUB6(:, repo))/(mean(HUB6(:, repo))+mean(HPB6(:, repo)));
    if (isnan(mean(HUB6(:, repo))/(mean(HUB6(:, repo))+mean(HPB6(:, repo)))))
        huproc_up(repo, 6) = 0;
    end
    buproc_up(repo, 1) = mean(BUB1(:, repo))/(mean(BUB1(:, repo))+mean(BPB1(:, repo)));
    if (isnan(mean(BUB1(:, repo))/(mean(BUB1(:, repo))+mean(BPB1(:, repo)))))
        buproc_up(repo, 1) = 0;
    end
    buproc_up(repo, 2) = mean(BUB2(:, repo))/(mean(BUB2(:, repo))+mean(BPB2(:, repo)));
    if (isnan(mean(BUB2(:, repo))/(mean(BUB2(:, repo))+mean(BPB2(:, repo)))))
        buproc_up(repo, 2) = 0;
    end
    buproc_up(repo, 3) = mean(BUB3(:, repo))/(mean(BUB3(:, repo))+mean(BPB3(:, repo)));
    if (isnan(mean(BUB3(:, repo))/(mean(BUB3(:, repo))+mean(BPB3(:, repo)))))
        buproc_up(repo, 3) = 0;
    end
    buproc_up(repo, 4) = mean(BUB4(:, repo))/(mean(BUB4(:, repo))+mean(BPB4(:, repo)));
    if (isnan(mean(BUB4(:, repo))/(mean(BUB4(:, repo))+mean(BPB4(:, repo)))))
        buproc_up(repo, 4) = 0;
    end
    buproc_up(repo, 5) = mean(BUB5(:, repo))/(mean(BUB5(:, repo))+mean(BPB5(:, repo)));
    if (isnan(mean(BUB5(:, repo))/(mean(BUB5(:, repo))+mean(BPB5(:, repo)))))
        buproc_up(repo, 5) = 0;
    end
    buproc_up(repo, 6) =  mean(BUB6(:, repo))/(mean(BUB6(:, repo))+mean(BPB6(:, repo)));
    if (isnan(mean(BUB6(:, repo))/(mean(BUB6(:, repo))+mean(BPB6(:, repo)))))
        buproc_up(repo, 6) = 0;
    end
...
% subplot(2,1,1);
% yyaxis left
bar([mean(oproc_up(:,1)), mean(ouproc_up(:,1)), mean(oproc_up(:,2)), mean(ouproc_up(:,2)), mean(oproc_up(:,3)), mean(ouproc_up(:,3)), mean(oproc_up(:,4)), mean(ouproc_up(:,4)), mean(oproc_up(:,5)), mean(ouproc_up(:,5)), mean(oproc_up(:,6)), mean(ouproc_up(:,6));
    mean(hproc_up(:,1)), mean(huproc_up(:,1)), mean(hproc_up(:,2)), mean(huproc_up(:,2)), mean(hproc_up(:,3)), mean(huproc_up(:,3)), mean(hproc_up(:,4)), mean(huproc_up(:,4)), mean(hproc_up(:,5)), mean(huproc_up(:,5)), mean(hproc_up(:,6)), mean(huproc_up(:,6));
    mean(bproc_up(:,1)), mean(buproc_up(:,1)), mean(bproc_up(:,2)), mean(buproc_up(:,2)), mean(bproc_up(:,3)), mean(buproc_up(:,3)), mean(bproc_up(:,4)), mean(buproc_up(:,4)), mean(bproc_up(:,5)), mean(buproc_up(:,5)), mean(bproc_up(:,6)), mean(buproc_up(:,6));
